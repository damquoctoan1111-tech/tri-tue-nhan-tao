...
@benchmark_timer
def ucs(start_state: BoardState, goal_state: BoardState, **kwargs):
    ...
    accumulated_cost = {start_state: 0}
    closed_set = set()
    nodes_generated = 1
    nodes_expanded = 0
    max_queue_size = 1
    ...
    while pq:
        ...
        current_g, _, curr_node = heapq.heappop(pq)
        ...
        if curr_node == goal_state:
            ...
            return build_result_pack(True, final_path, final_moves, nodes_expanded, nodes_generated, max_queue_size, "Tìm thấy lời giải bằng UCS.")
        ...
        for next_state, action_label, edge_cost in generate_adjacent_states(curr_node):
            ...
            if nxt_state := next_state:
                if nxt_state not in accumulated_cost or next_g < accumulated_cost[nxt_state]:
                    accumulated_cost[nxt_state] = next_g
                    visited_records[nxt_state] = (curr_node, action_label)
                    push_counter += 1
                    nodes_generated += 1
                    heapq.heappush(pq, (next_g, push_counter, nxt_state))
