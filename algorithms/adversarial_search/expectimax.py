...
@benchmark_timer
def expectimax(start_state: BoardState, goal_state: BoardState, heuristic='Manhattan', depth_limit=8, **kwargs):
    h_func = get_heuristic_function(heuristic, goal_state)
    decision, ext_nodes = _evaluate_game_tree_step(start_state, goal_state, h_func, min(depth_limit, 10), 'expectimax')
    if not decision:
    ...
    if hasattr(search_summary, 'status_message'):
        search_summary.status_message = 'Expectimax chọn nước đầu với chance node, sau đó A* hoàn tất đường đi.'
    else:
        setattr(search_summary, 'message', 'Expectimax chọn nước đầu với chance node, sau đó A* hoàn tất đường đi.')
