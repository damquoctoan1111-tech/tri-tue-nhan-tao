...
@benchmark_timer
def stochastic_hill_climbing(start_state: BoardState, goal_state: BoardState, heuristic='Manhattan', **kwargs):
    ...
    while nodes_expanded < node_limit:
        if curr == goal_state:
            return build_result_pack(True, current_path, action_history, nodes_expanded, nodes_generated, 1, "Tìm thấy lời giải bằng Stochastic Hill Climbing.")
        nodes_expanded += 1
        ...
        _, next_state, action_label = random.choice(candidates)
        curr = next_state
